...
1. **在线播放回退为直连源站 URL**：v4.0.0 引入的流代理（`/videos/stream/proxy`）在部分网络环境下仍存在兼容问题，导致在线播放失败。现已回退为原来的直连方式——播放器直接使用源站 CDN 视频 URL（与 v3.x 行为完全一致）。后端流代理端点保留（含 v4.0.1 的生命周期修复），后续如需可再启用。
