...
1. **满屏请求错误/超时提示**：修复搜索时可能触发大量请求错误提示导致满屏的问题，添加错误消息节流机制（相同错误2秒内只显示一次）

2. **网站无法获取视频（缓存空结果）**：修复后端 `lru_cache` 缓存失败/空结果导致后续所有请求返回空数据的问题，失败时不再缓存结果
...
3. **后端异常处理优化**：`video_service` 的 `search_videos`、`get_search_combination`、`get_home_data` 方法在失败时抛出异常而非返回空对象，避免错误结果被缓存
