...
1. **切换页面/刷新时提示"请求的资源不存在"**：添加了全局 API 404 处理器，未匹配的 `/api/*` 路由会记录警告日志并返回标准 JSON 响应，便于定位哪个端点缺失。同时前端 404 错误提示增加了请求 URL 信息，方便排查。

2. **下载页轮询日志噪音**：`/api/downloads/history` 每 5 秒轮询一次，日志中间件每次都记录。修复：日志中间件过滤掉高频轮询路径（成功时静默，失败时仍记录），大幅减少日志量。
