...
### 修改原因
1. 下载番剧时进度会超过视频真实大小（如 400M 视频下载到 440M 仍在继续），文件持续增长不停止。
2. 根因：下载循环没有按声明大小 `total_size` 封顶，服务器多传数据或链接无限推流时，单线程/分段下载都会把多余字节一直写入文件；单线程下载重试不重置已下载计数，导致进度跨重试累积虚高。
...
- `simple_download` 按 `total_size` 封顶：只接收剩余所需字节，超出部分直接丢弃，达到声明大小后立即停止读取并截断文件到实际大小。
- 每次重试重置已下载计数与已接收字节数，进度不再跨重试累积。
- 断点续传时若服务器忽略 Range（返回 200），自动回退为从头完整下载（`wb` 模式），避免把从头开始的数据追加到旧文件末尾。
- 断点续传遇到超过声明大小的旧文件时，截断到声明大小并直接标记完成。
- 若 GET 响应 `Content-Length` 大于 HEAD 探测值，以实际数据源为准更新总大小，避免反复"下载不完整"重试。

#### 2. 分段下载严格校验 206 并封顶

**文件：backend/app/services/download_service.py**
- `download_segment` 仅接受 206 响应；服务器忽略 Range（返回 200）时立即终止该段并记录错误，不再把整包数据写进段位置。
- 每段只接收段内剩余字节，超出部分丢弃；收到段范围全部字节后立即停止。
- 段下载结束后校验完整性，不足整段时报错重试。
- `segmented_download` 在所有段均因"服务器不支持分段下载"失败时，自动回退到单线程完整下载。
- 分段下载全部完成后将文件截断到 `total_size`，丢弃任何多余字节。

#### 3. 修复 Content-Range 解析

**文件：backend/app/services/download_service.py**
- 原逻辑 `if content_range and "/*" in content_range` 写反：只在总大小未知时尝试解析，已知总大小时反而跳过。改为解析 `bytes 0-8191/445644800` 这类格式中的总大小，`*`（未知）时跳过。

**文件：backend/app/models/download.py**
- `DownloadSegment` 新增 `error_message` 字段，用于记录段失败原因，支撑回退判断与后续排查。

#### 4. 版本号更新

**文件：backend/app/config.py**
- APP_VERSION 3.6.1 → 3.6.2
...
### 验证
- 编写本地模拟测试（C:\tmp\verify_download_fix.py，不依赖网络）：覆盖服务器多传数据、无限推流、重试计数重置、超大小旧文件续传截断、分段回退单线程、Content-Range 解析共 6 个场景，全部通过。
...
### 修改原因
1. 设置项"合集海报使用第一集海报"描述过长，需要精短表达
2. 刮削出来的评分全都是7.6，因为视频详情页 `get_video_detail()` 没有提取 `like_rate` 字段，导致评分始终回退到默认值 `DEFAULT_RATING = "7.6"`
...
**文件：frontend/src/views/Settings.vue**
- `label="合集海报使用第一集海报"` → `label="合集首集海报"`
...
- `get_video_detail()` 方法中新增 `like_rate` 提取逻辑（4策略依次尝试）：
  ...
  - 策略3：查找点赞按钮区域（`video-action` / `like-button` / `action-bar`）中的百分比
  - 策略4：宽松兜底，查找 class 含 `thumb`/`like`/`good` 的元素中的百分比
- `VideoDetail` 构造函数新增 `like_rate=like_rate` 参数

#### 3. 版本号更新

**文件：backend/app/config.py**
- `APP_VERSION` 从 `"3.5.0"` 更新为 `"3.5.1"`
...
### 影响
- 设置页面刮削区域的"合集海报使用第一集海报"显示为"合集首集海报"
- 刮削时如果能从详情页提取到点赞率（如83%），NFO评分将从7.6变为8.3等真实数值
- 如果源站详情页确实没有点赞率元素，评分仍为默认值7.6，不会比之前更差
