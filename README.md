...
5. **新增用户账户功能**:
   ...
   - 创建播放清单功能
   - 创建观看历史记录
   - 使用 SQLite 数据库持久化存储（可选 MySQL 云数据库）
...
9. **v4.0.0 大版本更新**:
   - **MinIO 对象存储**：封面/头像等展示图片可存储到 MinIO（项目专用 bucket，自动创建，失败自动降级本地；刮削 NFO/海报保持本地供 NAS 识别）
   - **观看历史续播**：播放进度自动上报，历史页显示进度与"继续观看"按钮，自动从上次位置续播
   - **流代理恢复**：在线视频流走后端代理通道，解决 CDN 直连被墙/失效
   - **追更订阅**：订阅番剧系列，新集提醒 + 一键下载最新集（本地/云数据库均支持）
   - **下载队列与并发限制**：`MAX_CONCURRENT_DOWNLOADS` 控制同时下载数，超出排队；播放清单一键全部下载
   - **媒体库海报墙**：已下载番剧大封面墙 + 本地评分/备注
   - **PWA 支持**：可添加到主屏幕、离线缓存
   - **管理员用户管理**：用户列表/添加/删除/重置密码/禁用/角色设置（管理员专属）
   - **服务端缓存**：视频元数据与翻译结果缓存，批量刮削大幅提速
   - **HLS（m3u8）下载**：自动用 ffmpeg 下载分段合并为 mp4
   - **下载中心增强**：总速度/预计剩余时间、下载完成通知
   - **登录体验**：登录后回到原页面；移除"记住密码"明文存储
   - **前端类型错误清零**：全部 TypeScript 错误修复，构建检查可通过
