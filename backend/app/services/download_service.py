...
class DownloadManager:
    ...
    async def _detect_series_directory(
        ...
        # 注意：OVA/特典/番外不再映射为99，统一按一季多集策略放入Season 1
        _LABEL_TO_SEASON_NUMBER = {
            ...
            '后篇': 3, '后編': 3, '下编': 3,
            # OVA/特典/番外：不再映射为99，全部归入Season 1按顺序分配集号
        }
        ...
        def _extract_series_base(title: str) -> str:
            ...
            patterns = [
                ...
                r'[・\s][赤青黒白紅緑黄紫黑绿蓝橙桃银金虹彩]$',  # 不潔之星・赤/黑/紫 -> 不潔之星
    ...
    def _extract_real_episode_number(self, title: str, subtitle: str = "") -> int:
        ...
        # 注意：OVA/特典/番外等类型标签不再映射为固定高集号(99)，
        # 而是返回0让系统按顺序分配，避免集号跳跃导致影视中心分页显示不佳
        _LABEL_TO_EPISODE = {
            '上卷': 1, '上巻': 1, '前篇': 1, '前編': 1, '上篇': 1,
            '中卷': 2, '中巻': 2, '中篇': 2, '中編': 2,
            '下卷': 3, '下巻': 3, '後篇': 3, '後編': 3, '下篇': 3,
            '后篇': 3, '后編': 3, '下编': 3,
            # OVA/特典/番外：返回0，由系统按下载顺序分配集号（E01, E02, ...）
        }
